...
const TakeQuiz = () => {
  ...
  const storedData = localStorage.getItem(
    `duration${
      Checkuser && Checkuser.account ? Checkuser.account._id : "questionn"
    }${id}`
  );
  if (!storedData) {
  ...
  useEffect(() => {
    if (!getidd) {
      if (Checkuser && Checkuser.account._id) {
        const storedData = localStorage.getItem(
          `questionn${
            Checkuser && Checkuser.account ? Checkuser.account._id : "questionn"
          }${id}`
  ...
  const SubmitAnswer = () => {
    ...
    dispatch(
      answerreturn({
        ...
        studentid:
          Checkuser && Checkuser.account._id ? Checkuser.account._id : "",
        subjectid: subject,
      ...
      {quizes && quizes.data ? (
        ...
        >
          <div className="mainLayoutc">
            <TopNavLog show={show} />
            {summary && summary.data && summary.data.notice===true ? (
              <div className="assesslayout">
                <p>You have already taken {fname}</p>
              </div>
            ) : (
              <>
                    ...
                    {questions.length > 0 ? (
                      <div className="assesslayoutquestion">
                        <div className="assesslayoutquestionh">
                          ...
                          <p style={{ color: "aqua",fontSize:"11px" }}>
