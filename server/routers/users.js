...
routers.route("/alluser").get(async (req, res) => {
  try {
    const allusers = await User.find({})
      .sort({ createdAt: "desc" })
      .populate({ path: "myfoundcards", populate: { path: "conversation" } })
...
routers.route("/searchmatch/:id").get(async (req, res) => {
  try {
   
    let ArrayReportedCards = [];
    let filteredList = [];
    const user_ac = await User.findById({ _id: req.params.id })
      .populate("myreportedcards");
    const cards = await CardModel.find({});
    if (user_ac) {
      if (user_ac.myreportedcards.length > 0) {
        ArrayReportedCards = user_ac.myreportedcards;
        if (cards && cards.length > 0) {
          cards.forEach((allcards) => {
            ...
            ArrayReportedCards.forEach((values) => {
              const report_firstname = values.firstname;
              const report_lastname = values.lastname;
              const report_middlename = values.middlename;
              const report_id = values.id_number;

              if (
                (p_firstname.toLowerCase()===(report_firstname.toLowerCase()) &&
                  p_lastname
                    .toLowerCase() === report_lastname.toLowerCase())
              ) {
                if(allcards.type===values.type){
